...
//    [item setBackgroundImage:[UIImage imageForResourcePath:@"MWPhotoBrowser.bundle/UIBarButtonItemGrid" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateNormal];
//    [item addTarget:self action:@selector(showGridAnimated) forControlEvents:UIControlEventTouchUpInside];
//    UIBarButtonItem * leftBarItem = [[UIBarButtonItem alloc]initWithCustomView:item];
//    
//     
//    UIBarButtonItem * leftItem =[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"MWPhotoBrowser.bundle/UIBarButtonItemGrid" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _item.frame = CGRectMake(0, 0, 30, 30);
        [_item setImage:[UIImage imageNamed:@"classification_1"] forState:UIControlStateNormal];
        [_item addTarget:self action:@selector(showGridAnimated) forControlEvents:UIControlEventTouchUpInside];
        _item.hidden = YES;
        
        _gridButton = [[UIBarButtonItem alloc]initWithCustomView:_item];
...
//                                                  object:_currentVideoPlayerViewController.moviePlayer];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(videoFinishedCallback:)
                                                 name:MPMoviePlayerPlaybackDidFinishNotification
                                               object:_currentVideoPlayerViewController.moviePlayer];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerStateChange:) name:MPMoviePlayerPlaybackStateDidChangeNotification object:_currentVideoPlayerViewController.moviePlayer];
...
- (void)playerStateChange:(NSNotification *)notic{
    switch (_currentVideoPlayerViewController.moviePlayer.playbackState) {
